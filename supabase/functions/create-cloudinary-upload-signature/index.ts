...
const cloudinaryUploadFolder = Deno.env.get("CLOUDINARY_UPLOAD_FOLDER") ?? "urban-parking/listing-photos";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!isUuid(body.parkingSpaceId)) {
      return invalid("Invalid parking space.");
    }
    ...
    const safePrefix = `${authData.user.id}/${body.parkingSpaceId}`;
