...
begin
  ...
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'parking_space_photos'
  ) then
    alter publication supabase_realtime add table public.parking_space_photos;
  end if;
end $$;

alter table public.parking_space_photos replica identity full;
...
comment on function public.get_public_parking_spot(uuid) is
  'Returns a safe public parking listing payload with ordered linked photo URLs, host contact metadata, and current-user ownership context for active spaces only.';
